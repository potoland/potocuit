...
import type { WorkerMessage, WorkerShardInfo, WorkerInfo } from './worker';
...
export class WorkersManger extends Map<number, Worker> {
	...
	async syncLatency({ shardId, workerId }: { shardId?: number; workerId?: number }) {
		...
		if (typeof shardId !== 'number' && typeof workerId !== 'number') { return; }

		const id = workerId ?? this.calculateWorkerId(shardId!);

		if (!this.has(id)) { throw new Error(`Worker #${workerId} doesnt exist`); }
	...
	async handleWorkerMessage(message: WorkerMessage) {
		switch (message.type) {
			...
			case 'CACHE_REQUEST': {
				...
				if (!worker) { throw new Error('Invalid request from unavailable worker'); }
				// @ts-expect-error
...
export type ManagerMessages = ManagerAllowConnect | ManagerSpawnShards | ManagerSendPayload | ManagerRequestShardInfo | ManagerRequestWorkerInfo | ManagerSendCacheResult;
