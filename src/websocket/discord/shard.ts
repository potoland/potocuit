...
export class Shard {
	...
	async onpacket(packet: GatewayReceivePayload) {
		...
		switch (packet.op) {
			...
			case GatewayOpcodes.InvalidSession:
				if (packet.d) {
					if (!this.resumable) { return this.logger.fatal(`[Shard #${this.id}] This is a completely unexpected error message.`); }
					await this.resume();
				} else {
	...
	async close(code: number, reason: string) {
		if (this.websocket?.readyState !== WebSocket.OPEN) { return this.logger.warn(`${new Error('418').stack} [Shard #${this.id}] Is not open`); }
